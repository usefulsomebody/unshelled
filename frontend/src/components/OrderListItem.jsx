...
export default function OrderListItem({ order, handleDelete }) {
  return (
    <div className="flex-table-item is-order">
      ...
      <div className="flex-table-cell">{format(new Date(order.date), 'yyyy-MM-dd')}</div>
      <div className="flex-table-cell cell-end action">
        <Link to={`/orders/${order.id}`} title="Modify">
          <svg
        ...
        <button data-action="delete" title="Delete" onClick={() => handleDelete(order.id)}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
